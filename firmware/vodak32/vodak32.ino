...
#define ADC_OFFSET     7    // correction offset for flat low end of ADC readings
...
void onSaveCfg(AsyncWebServerRequest *request){
  ...
  if(request->hasParam("vN", true) && volts_now != 0){  // calibrate with user vN value
		uint16_t vavg = 0;
		for(int c=0; c < 5; c++)
			vavg += analogRead(VOLT_SENSOR);
		volts_bits = (vavg/5 - ADC_OFFSET) / atof(request->getParam("vN", true)->value().c_str()); // bits per volt
  ...
  if(doSensorUpdate){ 
    ...
    for(int i = 0; i < SENSOR_COUNT; i++)
      if(sens_addrs[i][7]){ // // device family always non-zero when sensor exists
		...
		for(int x=0; x < 5; x++)
			vavg += analogRead(VOLT_SENSOR);
    volts_now = (vavg/5 - ADC_OFFSET) / volts_bits;
